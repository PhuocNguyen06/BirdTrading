...
export default function AdminShopOwnerPageController() {
   ...
   const { listSelected, currentPage } = useSelector(getShopOwnerTableSelector);
   ...
   const handleActionInRow = async (e) => {
      try {
         ...
         const res = await api.put("/admin/shop-owner/status", {
            ids: listSelected,
            status: e.target.value,
         });
         const data = await res.data;
         console.log(data);
         dispatch(getShopOwnerTablePagingAndFilter(currentPage));
         dispatch(adminShopOwnerSlice.actions.changeListSelectedRows([]));
            ...
            >
               ...
               >
                  <Typography sx={{ fontSize: "2rem", color: "red" }}>
                     Are you certain that you want to ban shop?
                  </Typography>
               </Box>
               <Box
                  sx={{
